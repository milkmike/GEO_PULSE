...
if page == "🌡️ Обзор":
    ...
    if _view == "Связи":
        ...
        _components.html(_get_threads(_viz), height=650, scrolling=False)
        st.stop()
    ...
    # ─── Map ───
    st.markdown(f'<div class="section-header">Карта отношений{info_badge("Цвет отражает «температуру» — взвешенный sentiment за выбранный период. Шкала: от −100° (враждебность, красный) до +100° (союзничество, зелёный). Нейтральные страны — жёлтые.")}</div>', unsafe_allow_html=True)
    ...
    for c in countries:
        if c.get("iso3"):
            ...
            map_data.append({
                ...
                "trend": c.get("trend") or "нет данных",
                "articles": c.get("article_count", 0),
            })
            if temp is not None:
                iso3_codes.append(c["iso3"])
            ...
            fig.add_trace(go.Choropleth(
                ...
                text=df_data.apply(
                    lambda r: f"<b>{r['country']}</b><br>"
                              f"Температура: {r['temperature']:.1f}°<br>"
                              f"Тренд: {r['trend']}<br>"
                              f"Статей: {r['articles']}",
                    axis=1,
                ),
        ...
        if not df_nodata.empty:
            fig.add_trace(go.Choropleth(
                ...
                text=df_nodata.apply(
                    lambda r: f"<b>{r['country']}</b><br>Нет данных<br>Статей: {r['articles']}",
                    axis=1,
                ),
                hoverinfo="text",
                colorscale=[[0, '#27272a'], [1, '#27272a']],
                showscale=False,
...
# ═══════════════════════════════════════════════════════════
    ...
    st.markdown("""
    <h2 style="font-size:1.2rem; font-weight:600; margin:28px 0 12px;">🔍 Что это?</h2>
    <p style="color:rgba(255,255,255,0.7); line-height:1.8; font-size:0.95rem;">
    GEO PULSE — аналитическая платформа, которая в реальном времени измеряет «температуру» двусторонних отношений между Россией и 10 странами постсоветского пространства. Система агрегирует медийный дискурс из 149 источников, верифицирует его данными ООН и торговой статистикой, и визуализирует динамику в виде единого числового показателя.</p>
    <p style="color:rgba(255,255,255,0.5); line-height:1.7; font-size:0.88rem; margin-top:8px;">
    Три слоя анализа: <strong>медийный sentiment</strong> (что пишут СМИ) → <strong>объективные данные</strong> (как голосуют в ООН, сколько торгуют) → <strong>корреляция</strong> (насколько медиа отражают реальность).</p>
