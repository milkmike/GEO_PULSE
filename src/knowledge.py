...
class KnowledgeBackfillService:
    ...
    def seed_registry(self, session: Any) -> int:
        ...
        for key in sorted(ENTITIES):
            ...
            self._entity_ids[key] = stored_uuid
    ...
    def backfill_batch(
        ...
        *,
        ...
        for row in rows:
            ...
            for key in legacy_entity_keys(values["entities"]):
                normalized_key = key.casefold()
                if normalized_key not in ENTITIES:
                    continue
                ...
                entity_id = self._entity_ids.get(normalized_key, mention.entity_id)
                session.execute(
                    text(
                        """
