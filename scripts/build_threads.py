...
VANCE_ALIASES = {
    "вэнс", "вэнса", "вэнсу", "вэнсом",
    "венс", "венса", "венсу", "венсом",
    "vance", "jd", "джейди", "джей", "ди"
}
...
Найди кластеры которые описывают ОДИН И ТОТ ЖЕ КОНКРЕТНЫЙ сюжет и должны быть объединены.

⚠️ СТРОГИЕ ПРАВИЛА:

МЕРЖИ ТОЛЬКО если:
- Один и тот же КОНКРЕТНЫЙ инцидент/событие (напр. "запрет въезда Сабурову" и "Россия запретила Сабурову въезд")
- Одно и то же лицо + действие, но разные написания (Вэнс/Венс, Сабуров/Saburov)
- Буквально та же новость с разных источников

НЕ мержи если:
- Разные события, даже если про одну тему или одну страну
- Общая тема совпадает, но конкретные события разные (напр. "визит Вэнса" и "экспорт газа" — это РАЗНЫЕ сюжеты, даже если оба про Азербайджан)
- Один кластер про дипломатию, другой про экономику — это разные сюжеты
- "Сотрудничество с ЕС по климату" и "Сотрудничество с НАТО" — РАЗНЫЕ, не мержить!

ТЕСТ: Если результат мержа будет содержать статьи про 3+ разных события — НЕ мержи.
Лучше оставить отдельные кластеры, чем создать кластер-помойку.
