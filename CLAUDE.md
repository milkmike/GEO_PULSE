...
> **Продукт публично называется «Массаракш»** (massaraksh.tech) — отсылка к
> Стругацким. Репозиторий и код остаются `GEO_PULSE` (имена менять не нужно).
> **Репозиторий публичный** — НЕ коммитить секреты и IP серверов: ключи только
> в `.env` (gitignored), доступ к проду — через SSH-алиас `geopulse-prod` из
> локального `~/.ssh/config` (не в репо).
...
- GDELT DOC API: оператор AND не поддерживается (термины AND-ятся неявно);
  ...
  IP может отдавать 503, при бёрстах — 429 (пейсинг GDELT_PAUSE_SEC, дефолт
  5с + бэкофф и circuit-breaker в scripts/collect_gdelt.py).
- Деплой: пуш в main → cron-автодеплой на сервере подтянет за ≤5 мин
  (deploy/auto-update.sh). Прямой доступ: `ssh geopulse-prod` (локальная
  машина оператора; облачные Claude-сессии исходящий SSH режут). Рабочее
  дерево на сервере держать чистым, серверные настройки — в .env.
